...
- destination -- path inside the target rootfs to extract the tarball to. The
...
func (pf *UnpackAction) Run(context *debos.Context) error {
	...
	if len(pf.Destination) > 0 {
		destDir, err = debos.RestrictedPath(context.Rootdir, pf.Destination)
		if err != nil {
			return err
		}
