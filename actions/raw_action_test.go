...
func TestRawAction_DefaultOrigin(t *testing.T) {
	...
	require.NoError(t, err, "Verify should pass without origin property")

	err = action1.Run(context)
	require.NoError(t, err, "Run should pass with default origin (recipe directory)")

	// Verify content was written to the image
	imageContent, err := os.ReadFile(imagePath)
	require.NoError(t, err)
	require.Equal(t, testContent, imageContent[:len(testContent)],
	...
	require.NoError(t, err, "Verify should pass with explicit 'recipe' origin")

	err = action2.Run(context)
	require.NoError(t, err, "Run should pass with explicit 'recipe' origin")
...
func TestRawAction_EmptySource(t *testing.T) {
	...
	require.Error(t, err, "Verify should fail when source is empty")
	require.Contains(t, err.Error(), "'source' property can't be empty")
}
...
func TestRawAction_InvalidOrigin(t *testing.T) {
	...
	err = action.Verify(context)
	require.NoError(t, err, "Verify should pass (origin is checked at runtime)")

	err = action.Run(context)
	require.Error(t, err, "Run should fail with missing origin")
	require.Contains(t, err.Error(), "origin `missing-origin` doesn't exist")
}
